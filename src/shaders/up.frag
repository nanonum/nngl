...
void main(void) {
  ...
  // float waveform = wave(st, (sin(u_time * 4.12) * .29), .250,  u_time * 12.520);
  ...
  //     .0001 + blur,  // * pow(pos.y, 1.2 * sin(u_time)),
  //     .0,
  //     smin(
  //         pow(periodicNoise21(pos, NUM_POLYGONS * 4.0 ), .80),
  //         pos.y * .580,
  //         2.50
  //     )
  // );
...
//   pos = xy2pol(st+ sin(u_time), len);
  vec3 colors = (grad(pos * 2.0+ sin(u_time * .2) * .40 ));
  // colors.g *= 0.50;
  fragColor = vec4(
    // colors + (1.0-balls) + (noise.x * .16) + clamp(highlight, -.10, .10) * .2,
