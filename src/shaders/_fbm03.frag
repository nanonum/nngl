...
void main(){
    ...
    float boost = .015;
    for(int i = 0; i < 7; i++) {
      float l_time = u_time / float(i);
      a *= 0.5;
      // pos.x += 0.2 * pos.y;
      pos.y += u_time * -float(i) * .02;
    //   pos.y += sin(pos.x * 8.0) * .02;
      pos.y += sin(pos.x * 3.280 * a+ pos.y * 3.80 * a) * .28 * a;
      pos *= 2.52;

    float modulator = pnoise31(vec3(pos * 4.0, a * 4.0 + l_time * 0.4));
    // modulator = 0.0;
      v += max(0.0, pnoise31(vec3(pos + a, a * 3.0 + l_time * 0.4 + modulator * .5)) * a) - length(st) * boost;
    }
    vec3 cloud = vec3(1.0 - smoothstep(8.0 * pow(v, 5.5), .67, v)); // cloudy
  
    // fragColor.gb *= .0029 * length(pos * 1.1);
    fragColor.rgb = vec3(1.0 -  smoothstep(0.3, 0.5, v));
    // fragColor.a = 1.0 -  smoothstep(0.4, 0.5, v);
    fragColor.a = 1.0;
}