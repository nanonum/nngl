...
float devi (in vec2 p) {
  vec2 i = fract(floor(p * u_resolution) / u_resolution) ;
  vec2 f = fract(p * u_resolution);

  float bias = .004 * tri(fract(u_time * .2))+ .005;
  vec4 ij = texture(u_texture_0, i, 0.0) ;
  ...
  vec4 avr = (a + b + c + d ) / 3.0;
  return ((ij.r - avr.r) * (ij.g - avr.g));
}
...
void main(){
  ...
  float aspect = u_resolution.y/u_resolution.x;
  vec2 st = gl_FragCoord.xy/u_resolution.x * .9;
  ...
  float texcel = devi(st * 1.0);
  vec3 co = vec3(
    fract(texcel)  * 1.0
  );

  fragColor = vec4(step(0.1,smoothstep(.0, 0.69, co)),  1.0);
  // fragColor = vec4(vec3(fract(co * 1.0)),  1.0);
}