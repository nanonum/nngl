...
vec4 blocksample (in vec3 p, in float size) {
  ...
  f = f + vec2(1.0, -1.0) * rot(f, 1.0 * u_time);
  float bias = 1.0 / size * .5;
...
void main(){
  ...
  vec3 stm = vec3(st.x * 1.0, st.y * 1.0, 1.0);
...
float lines = 0.0;
  ...
  for(int i = 0; i <51; i++) {
    float fi = float(i);
    float r = PI * .5 * fi + u_time * 3.2;
    // vec3 stm = translate(cos(r), random(vec2(u_time * .00004))) * stm;
    // lines += line(stm.x ) ;

    texcel = blocksample(stm * 1.0, .0 + fi * 3.250 + tan(u_time * .3) * 2.0);
// float draw_sin( vec2 st, float freq, float phase, float weight, float amp, float modulator){
