...
float line(float x) {
  return smoothstep(0.998, .999, fract(x));
...
void main(){
  ...
  vec3 stm = vec3(st.x * 4.0, st.y, 4.0);
...
float lines = 0.0;
  ...
  stm = translate(cos(r) - u_time, 02.0) * stm;
  // lines += line(stm.x);

  for(int i = 0; i < 100; i++) {
    float fi = float(i);
    float r = PI * .001 * fi + u_time * 0.01;
    vec3 stm = rot(r) * translate(cos(r), random(vec2(u_time * .01 + fi))) * stm;
    lines += pow(fract(line(cos(stm.x * 1.1450 )) * cos(stm.x) * fi) * .45, 1.42);
