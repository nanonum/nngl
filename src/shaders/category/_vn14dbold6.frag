...
float random (in vec2 st) {
  // return sin(sin(st.x + t * 1.0)* cos(st.y + t * -4.0));
    return sin(dot(st.xy, vec2(2.11)) * 8.0);
...
float noise (in vec2 p) {
  ...
  float biasX = 1.0; //sqrt(1.0 - f.x * f.x);
  float biasY = 1.0; //sqrt(1.0 - f.y * f.y);
...
void main(){
  ...
  st = rot(st, fract(5.0 * fract(noise(st + u_time * 0.2))));
  co += vec3(
    (noise(st * 20.0))
  );

  fragColor = vec4(step(.872 ,co),  1.0);
}