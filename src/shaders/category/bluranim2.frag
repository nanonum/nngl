...
uniform sampler2D u_texture_0; // ../../../public/SDIM7936.jpg
...
vec4 moz (in vec2 p) {
  float bit = sin(u_time * .3) * 12.0 + 63.0;
...
vec2 centeredST() {
  ...
  return ( 3.0 * gl_FragCoord.xy -u_resolution.xy)/ min(u_resolution.x, u_resolution.y) / aspect;
}
void main(){
  ...
  st = rot(st, noise(abs(st * 3.)) + u_time * 0.);
  vec4 texcel = moz(st * .59);
