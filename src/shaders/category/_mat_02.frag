...
float line(float x) {
  return smoothstep(0.982, 0.999995, fract(x));
...
void main(){
  ...
  vec3 stm = vec3(st.x * 25.0, st.y, 25.0);
...
float lines = 0.0;
  ...
  for(int i = 0; i < 3; i++) {
    float fi = float(i);
    float r = PI * .5 * fi + u_time * .012;
    vec3 stm = rot(r) * translate(cos(r * .1), random(vec2(length(st), fi))) * stm;
    lines += line(stm.x * cos(stm.y * .08)) * fi;
...
// float draw_sin( vec2 st, float freq, float phase, float weight, float amp, float modulator){
  ...
  fragColor = vec4(fract(rgb) * 3.0,  1.0);
}