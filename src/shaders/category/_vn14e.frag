...
i.y -= mod(floor(u_time * .4), 14.0);
  float hash = 7.0 + mod(floor(u_time * .4), 14.0);
  float a = random(i);
...
return fract( clamp(mix(mix(a, b, u[0]), mix(c, d, u[0]), u[1]), 0.2, .70) * (floor(fract(u_time * .2) * 6.0) + 2.0));
...
void main(){
  ...
  vec2 st = (70.0 * gl_FragCoord.xy -u_resolution.xy)/ min(u_resolution.x, u_resolution.y);

  // Use the noise function
  // n += step(.95,fract(noise(st) * 16.0)) * .02;
  vec3 co = vec3(
    (noise(vec2(st.x * .5, st.y * .5 + u_time)))
  );
  // co = noise_1 + noise_2;;

  fragColor = vec4(1.0 - step(0.2, co),  1.0);
}