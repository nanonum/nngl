...
void main(){
  ...
  co = rot3(co, 2.2);
   ...
   vec3 crt = vec3(
      co.r * 1.0 - getpartphasor(fract(uv.y * 2.0  + u_time), 4.0, 0.0, .40) * getpartphasor(fract(uv.x * 4.10), .10, fract(u_time* 0.05) + 4.11, .4),
      co.g * 1.0 - getpartphasor(fract(uv.y * 2.0  + u_time), 4.0, 1.0, .40) * getpartphasor(fract(uv.x * 4.10), .10, fract(u_time* 0.05) + 4.11, .4),
      co.b * 1.0 - getpartphasor(fract(uv.y * 2.0  + u_time), 4.0, 2.0, .40) * getpartphasor(fract(uv.x * 4.10), .10, fract(u_time* 0.05) + 4.11, .4)
    );
