...
vec4 sd (in vec2 p, in float div) {
  vec2 st = vec2(p.x, fract(p.y + .001 * hash11(u_time * -.02)));
...
void main(){
  ...
  for(int i = 0; i < loop; i++) {
    ...
    div *= pow(img.a, .10 + u_time * .00001)  * (2. + hash11(floor(u_time * .10)));
  ...
  fragColor.rgb = vec3(
    step(0.9995, fract(divst.x - st.x)) + 
    step(0.9995, fract(divst.y - st.y))
  ) * 1.;
