...
vec4 sd (in vec2 p, in float div) {
  vec2 st = vec2(p.x, fract(p.y + u_time * -.1));
  ...
  for(int i = 0; i < SAMPLES_SIZE - 1; i++) {
    // smp += texture(u_texture_0, st + fract((random(st.xy * float(i)) + fract(u_time * 0.02)) / div), 0.0);

    vec2 pointoffset = vec2(
      floor(float(i) * 0.5) / div * size + (0.5/div),
      float(i % 2) / div * size + (0.5/div)
    );
    vec4 smp = texture(u_texture_0, st + pointoffset, 0.0);

    // pointbuffer.rg += pointoffset;
    
    samples[i] = (length(smp.rgb));
    total += samples[i];
  }
  //addd center
  ...
  vec2 u = smoothstep(0.,1.,st) * st*st*(3.0-2.0*st);

  // return smin(mix(a, b, u[0]), mix(c, d, u[0]), u.x);
  // return ;

  float integ = mix(mix(samples[0], samples[1], u[0]), mix(samples[2], samples[3], u[0]), u[1]);
  ...
  for(int i = 0; i < SAMPLES_SIZE; i++) {
    ...
    sigma += pow(samples[i] , 2.0);
  }
  sigma /= float(SAMPLES_SIZE);

  float sd = sqrt(sigma) - pow(integ, 2.0);
  return vec4(
