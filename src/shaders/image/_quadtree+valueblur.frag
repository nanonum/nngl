...
vec4 blur (in vec2 p, in float size) {
  ...
  float bias = 1.0 / size;
  vec4 a = texture(u_texture_0, i + vec2(cos(-bias), cos(-bias)), 0.0);
  vec4 b = texture(u_texture_0, i + vec2(sin(bias),cos(-bias)), 0.0);
  vec4 c = texture(u_texture_0, i + vec2(cos(-bias), sin(bias)), 0.0);
  vec4 d = texture(u_texture_0, i + vec2(sin(bias), sin(bias)), 0.0);
...
vec4 sd (in vec2 p, in float div) {
  vec2 st = vec2(p.x, fract(p.y + u_time * -.02));
  ...
  vec4 point = mix(mix(texcels[0], texcels[1], u[0]), mix(texcels[2], texcels[3], u[0]), u[1]);
